...
void Ship::Shoot()
{
    ...
    for (int i = 1; i <= nProjectiles; i++) {
        ...
        Game::Instance().InstantiateGameObject(new Projectile(damage), new Transform(glm::vec3(pCoords.first, pCoords.second, 0.0f), glm::vec3(this->transform.rotation.x, this->transform.rotation.y, this->transform.rotation.z + currentAngle), glm::vec3(0.10f, 0.25f, 0.25f)));
    }
}
