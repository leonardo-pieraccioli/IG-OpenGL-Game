...
{
    static auto&& instance = TextManager();
    return (instance);
}

void TextManager::InitManager(int ScreenWidth, int ScreenHeight)
{
    // setting up text projection matrix
...
int TextManager::LoadFont(const char* fontName, std::string name)
{
    auto it = Fonts.find(name);

    if (it != Fonts.end()) {
        std::cout << "Font già caricato in precedenza" << std::endl;
        return 0;
    }
...
void TextManager::RenderText(std::string text, float x, float y, float scale, glm::vec3 color, std::string fontName)
{
    ...
    std::map<GLchar, Character> Characters = Fonts[fontName];
